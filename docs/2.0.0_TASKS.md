...
- [ ] `DashboardViewModel` - Instance list and quick actions (merged into MainViewModel)
- [ ] `InstallWizardViewModel` - Installation workflow
- [ ] `InstanceDetailViewModel` - Single instance management (merged into WslInstanceViewModel)
...
- [ ] `DashboardPage.xaml` - Home page with instance cards (implemented in MainWindow)
...
#### 2.4.4 UI Components
- [ ] `InstanceCard` - UserControl for instance display (implemented inline in MainWindow)
- [ ] `PackageListItem` - UserControl for package row (implemented inline in PackageManagerPage)
