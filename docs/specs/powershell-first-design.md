...
| Network, firewall, diagnostics | `network.status.v1`, `network.ip.v1`, `network.port-mappings.v1`, `network.probe.v1`, `network.mode.get.v1`, `network.mode.preview.v1`, `network.mode.set.v1`, `network.settings.preview.v1`, `network.settings.set.v1`, `firewall.list.v1`, `firewall.preview-create.v1`, `firewall.create.v1`, `firewall.preview-remove.v1`, `firewall.remove.v1`, `diagnostics.preview.v1`, `diagnostics.export.v1` | New commands use `Get`/`Test` for reads, `Set`/`New`/`Remove` for changes, and `Export-DistroNexusDiagnosticReport` for report output. The existing port-mapping cmdlet becomes a fixed Bridge client rather than direct host execution. | Mode/settings/firewall changes preserve preview tokens and elevation grants; diagnostic export accepts only selected allowed/redacted inputs and a preview token, with `ShouldProcess`. |
