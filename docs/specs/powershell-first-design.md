...
| FR-003, FR-007 | Existing Podman public commands expose Core-issued, single-use previews and use `ShouldProcess`; Core owns endpoint validation, fingerprints, expiry, and replay checks. | Preserve the existing typed preview object/token unchanged. The client may pass only explicit fields and returned typed preview/token values; it may not construct a token, submit arbitrary command text, or weaken Core endpoint validation. | Pester `WhatIf`/invalid-input/replay coverage and xUnit client serialization tests. |
