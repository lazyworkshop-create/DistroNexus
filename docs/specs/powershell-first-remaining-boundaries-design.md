...
The ready broker-free subset uses only `usb.status.v1 {}` and `usb.list.v1 {}`; both reject every payload field. The exports/client methods are `Get-DistroNexusUsbStatus` / `GetUsbStatusAsync(CancellationToken)` and the compatible `Get-DistroNexusUsbDevice` / `GetUsbDevicesAsync(CancellationToken)`. `UsbStatusResult { IsInstalled, ServiceState, Version?, SupportsActions, Reason?, OutcomeCode }` and `UsbDeviceListResult { Devices[<=128], OutcomeCode }` are closed records whose serialized response is capped at 64 KiB. `ServiceState` is exactly `Running`, `Stopped`, or `Unknown`; `Availability` is exactly `Available`, `Shared`, `Attached`, `NotConnected`, `Unsupported`, or `Unknown`; `SharedState` and `AttachedState` are booleans. `OutcomeCode` is one of `Usb.Ready`, `Usb.NotInstalled`, `Usb.ServiceUnavailable`, `Usb.ListUnavailable`, `Usb.ListMalformed`, or `Usb.BridgeUnavailable`. `Version` is nullable and canonical `[0-9]{1,5}(\.[0-9]{1,5}){0,3}` (24 characters maximum); `Reason`, `Description`, `Distribution`, and `Guidance` are display-safe strings capped at 256 characters. Each `UsbDeviceResult` contains only `BusId`, `Description`, `Availability`, `SharedState`, `AttachedState`, `IsStorage`, `Distribution?`, and `Guidance?`; identifiers use the existing USB bus-id grammar, and no raw path, command, endpoint, diagnostic, action grant, or broker proof crosses the boundary. The typed client rejects unknown fields, invalid enum/boolean combinations, over-limit items/text, and oversized envelopes.

Core/Bridge owns the fixed adapter invocation and sanitization. `usb.list.v1` independently rechecks usbipd installation/service state before listing; if unavailable it returns an empty list with the matching stable status outcome. Desktop replaces its watcher with a single cancellable poll loop only while the USB view is visible, with one in-flight request and a minimum 2-second interval; its unload/dispose owner cancels the loop and suppresses every completion that races after unload. The Desktop may render the module-returned `SupportsActions` value and stable guidance, but does not invoke attach/detach.

`usb.action.preview.v1` and `usb.action.execute.v1` remain blocked. Bind/unbind can only complete when the release/security-owned signed broker contract is available; no module, Desktop, helper, or generic elevation fallback may substitute for that contract. The existing direct legacy connect/disconnect exports are not an approved PowerShell-first action boundary and are excluded from S45a; their retirement or replacement requires a separately authorized action-closure slice. No USB action methods are added to the Desktop client in this slice.
...
### Diagnostics and final Desktop boundary

`diagnostic.snapshot.v1 {}` is a fixed empty-payload read: the Bridge rejects any supplied field before probing. Its export/client pair is `Get-DistroNexusDiagnosticSnapshot` with no parameters / `GetDiagnosticSnapshotAsync(CancellationToken)`, and the command is exported in the manifest. The closed result is `DiagnosticSnapshotResult { ModuleState, WslState, BridgeState, Notices[<=16], OutcomeCode }`: `ModuleState` and `BridgeState` are literal `Ready` for every successful snapshot response, while `WslState` is exactly `Ready`, `Unavailable`, or `Unknown`. `Notices` are `{ Code, Severity, Message }` with `Code` ASCII `[A-Z][A-Za-z0-9.]{0,63}`, `Severity` exactly `Info`, `Warning`, or `Error`, and a nonempty trimmed display-safe `Message` of at most 256 characters with control characters removed. The UTF-8 envelope is at most 16 KiB. The only route outcomes are `Diagnostic.Ready` when `WslState=Ready`, and `Diagnostic.Degraded` when `WslState=Unavailable|Unknown`. Bridge/module transport unavailability remains the existing typed-client transport failure, not a fabricated snapshot result.

Core derives `WslState` from `IPlatformCapabilityService.GetHostSnapshotAsync` and its fixed `CapabilityId.Wsl` record; a missing or failed capability record maps to `Unknown`, an unsupported record maps to `Unavailable`, and a supported record maps to `Ready`. Core catches host errors before projection; it never returns paths, command lines, raw exceptions, environment variables, module timestamps, or serialized host output. The typed client rejects unknown fields, invalid state/outcome combinations, over-limit envelopes/notices, and cancellation is propagated to the fixed request.

`MainViewModel.ShowDiagnosticsAsync` renders only this typed snapshot. Startup uses this order: establish product-independent default culture and logging without any product settings path/input; construct the host and typed client; call `GetBootstrapSettingsAsync` exactly once before settings/update consumers; then apply only the returned typed settings to UI culture/logging options. `LogPath` is never a Desktop input. `InitializePowerShellModule`, every raw `settings.json` read, and their relative-path/fallback branches are deleted. `IPowerShellService` remains permitted solely as the private App-to-`PowerShellModuleClient` composition dependency; no view model or window resolves it. The Desktop composition removes every stale business-service registration and constructor parameter whose consumer has migrated to the client.

S46a's `S13DesktopCompositionAndLocalizationTests` scans `App.xaml.cs`, `MainViewModel.cs`, `WslInstanceViewModel.cs`, `InstanceDetailViewModel.cs`, and `SettingsViewModel.cs` plus all DI registration edges. It forbids Core business-service interfaces, Bridge operation identifiers, and `IPowerShellService` outside the App-to-client composition edge; it additionally asserts the only retained composition registration is `IPowerShellService -> PowerShellModuleClient`. The same test owns this explicit S46b closure inventory:

| Source | Current classification | Required S46b closure |
| --- | --- | --- |
| `ViewModels/TemplatesViewModel.cs` | Raw selected-file content parsing | Typed local-template import input contract |
| `Wizard/Steps/ProgressStep.cs` | Product log path propagation | Typed product log target result |
| `Wizard/Steps/ResultStep.cs` | Product log directory creation/reveal | Core-owned target creation and returned safe reveal target |
| `Controls/Tabs/IntegrationsTabView.xaml.cs` | Raw external launch | Module-returned validated URI plus narrow UI launcher |

S46a acceptance requires that finite inventory to be asserted as outstanding, never silently excepted. S46b removes every entry and makes the global inventory empty. The final permitted UI exceptions are WPF rendering/navigation/dialog/clipboard, picker selection without product parsing, and browser/Explorer launch of a module-returned display-safe target.
