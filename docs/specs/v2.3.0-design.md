...
Workspace commands use the packaged `DistroNexus.WorkspaceBridge` newline-delimited JSON protocol and keep one bridge process for the module session so preview/import/launch tokens remain valid. They never read or write `workspaces.json` directly. The resolver accepts only the package's known bridge DLL, or the fixed repository Debug build output while developing; environment path overrides and scripts/binaries are rejected. Workspace mutators require `-ExpectedRevision` and pass it to Core for optimistic-concurrency checks. A new definition, including an import, uses entity revision `0`; an import preview also binds the token to the versioned `workspaces.json` document revision, so any intervening store write makes that preview stale. Duplicate uses the selected source definition's revision. Import and launch additionally require the corresponding Core-issued preview token.
