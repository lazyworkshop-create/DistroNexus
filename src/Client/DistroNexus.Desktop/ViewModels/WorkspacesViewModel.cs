...
public sealed partial class WorkspacesViewModel : ObservableObject
{
    ...
    public WorkspacesViewModel(IPowerShellModuleClient workspaces, IDialogService? dialogs = null, WorkspaceStartupRequest? startupRequest = null) => (_workspaces, _dialogs, _startupRequest) = (workspaces, dialogs, startupRequest);
    ...
    [RelayCommand] private async Task CreateShortcutAsync() { if (SelectedWorkspace is null) return; try { var result = await _workspaces.CreateWorkspaceShortcutAsync(SelectedWorkspace.Id); Status = result.OutcomeCode == "Workspace.ShortcutCreated" ? L("Workspace_StatusShortcut", "Shortcut created.") : result.OutcomeCode; } catch (Exception ex) { Status = MainViewModel.FormatAlertMessage(ex); } }
