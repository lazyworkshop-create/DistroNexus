...
function Open-DistroNexusNetworkLoopback { [CmdletBinding(SupportsShouldProcess)] param([Parameter(Mandatory)][ValidateSet('localhost','127.0.0.1','::1')][string]$Host,[Parameter(Mandatory)][ValidateRange(1,65535)][int]$Port) if ($PSCmdlet.ShouldProcess("$Host`:$Port",'Open fixed loopback HTTP endpoint')) { Invoke-DistroNexusWorkspaceBridge -Operation 'browser.loopback.v1' -Payload @{ Host=$Host; Port=$Port } } }
