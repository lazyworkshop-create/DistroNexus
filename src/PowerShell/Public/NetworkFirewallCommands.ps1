...
function Set-DistroNexusNetworkMode { [CmdletBinding(SupportsShouldProcess)] param([Parameter(Mandatory)][ValidatePattern('^[a-zA-Z0-9-]{16,128}$')][string]$PreviewToken) if ($PSCmdlet.ShouldProcess('reviewed WSL networking mode','Apply reviewed WSL networking mode')) { Invoke-DistroNexusWorkspaceBridge -Operation 'network.mode.set.v1' -Token $PreviewToken } }
function Get-DistroNexusNetworkSettings { [CmdletBinding()] param() Invoke-DistroNexusWorkspaceBridge -Operation 'network.settings.get.v1' }
...
function Set-DistroNexusNetworkSettings { [CmdletBinding(SupportsShouldProcess)] param([Parameter(Mandatory)][ValidatePattern('^[a-zA-Z0-9-]{16,128}$')][string]$PreviewToken) if ($PSCmdlet.ShouldProcess('reviewed network settings','Apply reviewed WSL network settings')) { Invoke-DistroNexusWorkspaceBridge -Operation 'network.settings.set.v1' -Token $PreviewToken } }
function Open-DistroNexusNetworkLoopback { [CmdletBinding(SupportsShouldProcess)] param([Parameter(Mandatory)][Alias('Host')][ValidateSet('localhost','127.0.0.1','::1')][string]$TargetHost,[Parameter(Mandatory)][ValidateRange(1,65535)][int]$Port) if ($PSCmdlet.ShouldProcess("$TargetHost`:$Port",'Open fixed loopback HTTP endpoint')) { Invoke-DistroNexusWorkspaceBridge -Operation 'browser.loopback.v1' -Payload @{ Host=$TargetHost; Port=$Port } } }
