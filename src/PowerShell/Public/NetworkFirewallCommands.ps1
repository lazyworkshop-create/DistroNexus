...
function Test-DistroNexusNetworkProbe {
    [CmdletBinding()] param([Parameter(Mandatory)][ValidateSet('Dns','Gateway','Internet','WindowsHost','WslInstance','Localhost','TcpEndpoint')][string]$Kind,[Parameter(Mandatory)][ValidatePattern('^[A-Za-z0-9][A-Za-z0-9.-]{0,252}$')][string]$Host,[ValidateRange(1,65535)][int]$Port,[ValidateRange(1,30)][int]$TimeoutSeconds=5,[ValidatePattern('^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$')][string]$DistributionName)
    $request=@{ Kind=$Kind; Host=$Host; Timeout="00:00:$TimeoutSeconds" }; if ($PSBoundParameters.ContainsKey('Port')) { $request.Port=$Port }; if ($PSBoundParameters.ContainsKey('DistributionName')) { $request.DistributionName=$DistributionName }; Invoke-DistroNexusWorkspaceBridge -Operation 'network.probe.v1' -Payload @{ Request=$request }
}
