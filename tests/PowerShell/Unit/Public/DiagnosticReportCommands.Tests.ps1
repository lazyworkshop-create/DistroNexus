...
Describe 'Diagnostic report bridge contract' -Tag 'Unit', 'Public', 'Diagnostic' {
    ...
    It 'maps preview and redacted export to fixed versioned routes' {
        InModuleScope DistroNexus {
            ...
            Export-DistroNexusDiagnosticReport -SnapshotToken ('a' * 32) -DestinationFileName report.json -DeadlineMilliseconds 100 -Confirm:$false | Out-Null
            ...
            Should -Invoke Invoke-DistroNexusWorkspaceBridge -Times 1 -Exactly -ParameterFilter { $Operation -eq 'diagnostics.export.v1' -and $Token -eq ('a' * 32) -and $Payload.DestinationFileName -eq 'report.json' -and $Payload.DeadlineMilliseconds -eq 100 }
        }
    }
    ...
    It 'does not invoke export when WhatIf is requested' {
        InModuleScope DistroNexus {
            ...
            Export-DistroNexusDiagnosticReport -SnapshotToken ('a' * 32) -DestinationFileName report.md -WhatIf | Should -BeFalse
    ...
    It 'does not invoke export when confirmation is declined' {
        ...
        $script = "Import-Module '$modulePath' -Force; & (Get-Module DistroNexus) { function Invoke-DistroNexusWorkspaceBridge { exit 9 }; Export-DistroNexusDiagnosticReport -SnapshotToken $('a' * 32) -DestinationFileName report.json -Confirm }"
    ...
    It 'propagates only the bridge sanitized diagnostic failure' {
        InModuleScope DistroNexus {
            ...
            { Export-DistroNexusDiagnosticReport -SnapshotToken ('a' * 32) -DestinationFileName report.json -Confirm:$false } | Should -Throw '*Diagnostic.ExportInvalid: <redacted>*'
        }
    }
    ...
    It 'rejects an unsafe destination before bridge invocation' {
        InModuleScope DistroNexus {
            Mock Invoke-DistroNexusWorkspaceBridge { throw 'Bridge must not be called.' }
            { Export-DistroNexusDiagnosticReport -SnapshotToken ('a' * 32) -DestinationFileName '..\report.json' -Confirm:$false } | Should -Throw '*file name*'
            Should -Invoke Invoke-DistroNexusWorkspaceBridge -Times 0 -Exactly
        }
    }
}
